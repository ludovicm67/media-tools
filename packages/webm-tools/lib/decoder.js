...
import { readDate, readFloat, readHexString, readSigned, readUnsigned, readUtf8, readVint, writeSigned, writeUnsigned } from './utils.js'
...
const readTag = (ctx) => {
  ...
  const tagStr = readHexString(ctx.get('buffer'), ctx.get('cursor'), ctx.get('cursor') + length)
...
const readDataFromTag = (ctx, tagObj, data) => {
  ...
  if (fixTimestamps && name === 'Timecode') {
    ...
    value = isFirstBlock ? lastTimestampValue : (lastTimestampValue + ctx.get('timestampDelta'))
